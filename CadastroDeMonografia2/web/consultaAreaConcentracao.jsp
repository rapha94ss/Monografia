...
<html>
    ...
    <body>
        <div class="container">
            ...
            <h1>Consulte por Area de Concentração</h1>
        <select name="selecao" id="selecao" >
           <option value="0">Escolha uma Area de Concentração</option>
           <%
               String resp = "";
               try{
                   ResultSet rs = stmt.executeQuery("SELECT * FROM monografias ORDER BY area_concentracao");
                   while(rs.next()){
                       resp+="<option value='"+rs.getInt("id")+"'>";
                       resp+=rs.getString("area_concentracao")+"</option>";
                   }
                   rs.close();
               }catch(Exception ex){
